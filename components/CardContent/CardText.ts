...
function CardText() {
    ...
    const titlePosts = Multilanguage(
        "React app + Tailwind Css + C# .Net Core 6 Web API + Postgresql in Docker"
    );
    const altPosts = Multilanguage("posts");
    const descriptionPosts = Multilanguage(
        "Я розробив Full stack програму, в на базі бібліотеки React і для візуальної частини я використовува Tailwind Css, для серверної частини я використовував .NET Web API і для збереження даних баду даних Postgresql. Також я додав файли Docker i docker-compose.yml для перегляду просграми в докер контейнері . Є можливість створення нового користувача із фото, фото я зберігаю на сервері. Користувач може переглянути всі пости, а також створити свій власний також із картинкою. Користувач має можливість коментувати пости, додавати вподобайку або забирати, відповідати на коментарі. Всі пости або коментарі створенні даним користувачем можуть редагуватись або видалятись. Авторизація користувача зберігається на протязі доби.",
        "I developed a full stack program, based on the React library and for the visual part I use Tailwind Css, for the server part I used .NET Web API and for saving data a Postgresql database. I also added Docker and docker-compose.yml files to view the program in the docker container.. It is possible to create a new user from a photo, I save the photo on the server. The user can view all the posts and also create his own also with a picture. The user has the opportunity to comment on posts, add or remove favorites, reply to comments. All posts or comments created by this user can be edited or deleted. User authorization is stored for 24 hours.",
